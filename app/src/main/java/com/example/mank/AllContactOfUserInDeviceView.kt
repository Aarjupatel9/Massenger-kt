...
//                    prevCatchText = newText;
    ...
    private fun start() {
        if (ContextCompat.checkSelfPermission(
            ...
            MainActivity.db?.let { syncContactDetails(it) }
        } else {
    ...
    private fun askPermission(Manifest_request: String) {
        if (Manifest_request == Manifest.permission.READ_CONTACTS) {
            ActivityCompat.requestPermissions(
                this, arrayOf(
                    Manifest.permission.READ_CONTACTS
                ), permission_code.CONTACTS_PERMISSION_CODE
            )
        }
    }

    private val allContactOfUserEntityList: List<AllContactOfUserEntity?>? = ArrayList()
    ...
    private val allContactOfUserEntity: AllContactOfUserEntity? = null
    fun syncContactDetails(db: MainDatabaseClass) {
        getUserContactDetailsFromPhone =
            GetUserContactDetailsFromPhone(this@AllContactOfUserInDeviceView, db)
    ...
    private fun syncContactListToServer(
        ...
        val scdt = SyncContactDetailsThread(this, connectedContact, disConnectedContact, object :
            ...
            override fun execute(status: Int, massege: String?) {
                ...
                Toast.makeText(this@AllContactOfUserInDeviceView, massege, Toast.LENGTH_LONG).show()
                if (status == 1) {
    ...
    fun syncContactListToServerCallBack() {
        synchronized(this) {
            val newContactDetailsHolder = MainActivity.db?.let { contactDetailsHolderForSync(it) }
...
private fun setContentDetailsInView() {
    synchronized(this) {
        ...
        val uniqueContacts = TreeSet<AllContactOfUserEntity>(contactComparator)
        disConnectedContact?.filterNotNull()?.let { uniqueContacts.addAll(it) }
        ...
        connectedContact?.filterNotNull()?.let { uniqueContacts2.addAll(it) }
        ...
        connectedContact?.filterNotNull()?.let { contactArrayList?.addAll(it) }
        ContactSyncMainRecyclerViewAdapter!!.notifyDataSetChanged()
        Log.d(
            "log-AllContactOfUserDeviceView",
        ...
        disConnectedContact?.filterNotNull()?.let { contactArrayList!!.addAll(it) }
        ContactSyncMainRecyclerViewAdapter!!.notifyDataSetChanged()
...
override fun onRequestPermissionsResult(
    ...
    if (requestCode == permission_code.CONTACTS_PERMISSION_CODE) {
