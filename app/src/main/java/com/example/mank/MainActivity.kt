...
class MainActivity : FragmentActivity() {
    ...
    override fun onStart() {
        ...
        Log.d(
            ...
            "onStart: in MainActivity Contact_page_opened_id  is  : " + Contact_page_opened_id
        )
    ...
    fun verifyLogin(code: Int) {
        ...
        if (!permissionMain.hasPermissions(this, *permission_code.PERMISSIONS)) {
            ActivityCompat.requestPermissions(
                ...
                permission_code.PERMISSION_ALL
            )
        }
    ...
    fun syncContactAtAppStart() {
        if (!permissionMain.hasPermissions(this, *permission_code.CONTACT_STORAGE_PERMISSION)) {
            ActivityCompat.requestPermissions(
                this,
                permission_code.PERMISSIONS,
                permission_code.PERMISSION_CONTACT_SYNC
            )
            return
        }
        val tf = Thread {
            try {
                ...
                Log.d(
                    "log-MainActivity",
                    "Thread tf || lastOpenTime : " + insertedAtLastEntity?.date
                )
                if (insertedAtLastEntity?.date!! < Date().time - 300000) {
...
//                        Toast.makeText(MainActivity.this, "app is opened after 1 min break or more", Toast.LENGTH_SHORT).show();
        ...
        val scdt = SyncContactDetailsThread(this, connectedContact, disConnectedContact, object :
            IContactSync {
...
//        Toast.makeText(this, "you Click all contact details", Toast.LENGTH_SHORT).show();
    ...
    private fun CreateSocketConnection() {
        ...
        socket!!.on(Socket.EVENT_CONNECT) {
            ...
            try {
                ...
                if (tmp3 != null) {
                    for (tmp1 in tmp3) {
                        try {
                            ...
                            Log.d(
                                "log-onJoinAcknowledgement",
                                "exception || massegeData : $massegeData"
                            )
                            Log.d(
                                "log-onJoinAcknowledgement",
                                "exception || tmp3.size() : " + massegeData.length()
                            )
                        }
                    }
                }
                if (tmp3?.size!! > 0) {
                    ...
                    Log.d(
                        "log-MainActivity",
                        "onJoinAcknowledgement || massegeData size :$massegeData"
                    )
                }
            } catch (e: Exception) {
        ...
        socket!!.on(
            ...
            onMassegeReachAtServerFromCMDV
        )
                    ...
                    if (profileImageByteArray.size > 0) {
                        synchronized(this) {
                            ...
                            Log.d(
                                ...
                                "Saved image of size : " + profileImageByteArray.size + " and resolution : " + bitmapImage.width + "*" + bitmapImage.height
                            )
                            contactListAdapter!!.practiceMethod(
                                id,
                                profileImageByteArray
                            ) // to update contactList
                            if (saveContactProfileImageToStorage(id, profileImageByteArray)) {
                                massegeDao!!.updateProfileImageVersion(
                                    id,
                                    ProfileImageVersion,
                                    user_login_id
                                )
    ...
    private val onMassegeReachReadReceipt = Emitter.Listener { args ->
        ...
        Log.d(
            "log-onMassegeReachReadReceipt",
            "onMassegeReachReadReceipt || start requestCode:$requestCode"
        )
        if (requestCode == 1) {
            ...
            try {
                ...
                if (Contact_page_opened_id == receiver_id) {
                    ...
                    ContactMassegeDetailsView.massegeListAdapter?.updateMassegeStatus(
                        receiver_id,
                        massege_sent_time,
                        viewStatus
                    )
                ...
                val t = Thread {
                    val massegeStatus = massegeDao!!.getMassegeStatus(
                        sender_id,
                        receiver_id,
                        massege_sent_time,
                        user_login_id
                ...
                try {
                    ...
                    Log.d(
                        "log-onMassegeReachReadReceipt",
                        "massege_reach_read_receipt_acknowledgement event emitted"
                    )
                } catch (e: Exception) {
    ...
    private val onMassegeArriveFromServer = Emitter.Listener { args ->
        ...
        Log.d(
            ...
            "onMassegeArriveFromServer || requestCode is : $requestCode"
        )
        ...
        if (requestCode == 0) {
            ...
            try {
                ...
                for (i in contactArrayList1!!.indices) {
                    if (contactArrayList1[i]?.CID == new_massege_sender_id) {
                        //means massege arrive from known or saved contacts
                        Log.d(
                            "log-onMassegeArriveFromServer3",
                            "massege arrive from known sources : "
                        )
                        if (new_massege_sender_id == Contact_page_opened_id) {
                            Log.d("log-onMassegeArriveFromServer3", "Contact page is opened")
                            viewStatus = 3
                            val newMassegeEntity2 = MassegeEntity(
                                new_massege_sender_id,
                                user_login_id,
                                new_massege["massege"].toString(),
                                new_massege_time_of_send,
                                viewStatus
                            )
                            ContactMassegeDetailsView.massegeListAdapter?.addMassege(
                                newMassegeEntity2,
                                1
                            ...
                            runOnUiThread {
                                recyclerViewAdapter!!.notifyDataSetChanged()
                                ChatsRecyclerView!!.scrollToPosition(
                ...
                val checkContactSavedInDB = Thread {
                    val x =
                        massegeDao!!.getContactWith_CID(newMassegeEntity1.senderId, user_login_id)
                    if (x == null) {
                        ...
                        val setPriorityRankThread = Thread {
                            val HighestPriority = massegeDao!!.getHighestPriorityRank(
                                user_login_id
                            )
                            ...
                            if (contactListAdapter != null) {
                                contactListAdapter!!.updatePositionOfContact(
                                    newMassegeEntity1.senderId,
                                    this@MainActivity
                                )
                            }
                ...
                try {
                    ...
                    if (new_massege_sender_id != user_login_id) {
                        socket!!.emit(
                            ...
                            user_login_id,
                            jsonArray
                        ) // ViewStatus 2 means at contact's database and 3 means read by contact
                    }
                    Log.d(
                        "log-onMassegeArriveFromServer3",
                        "massege_reach_read_receipt_acknowledgement socket emit :$jsonArray"
                    )
                ...
                Log.d(
                    "log-onMassegeArriveFromServer-Exception",
                    "call: error while parsing data : $e"
                )
            }
        } else {
            ...
            Log.d(
                "log-onMassegeArriveFromServer",
                "onMassegeArriveFromServer unHandled requestCode arrive : $requestCode"
            )
        }
    }
    ...
    private val onMassegeReachAtServerFromCMDV = Emitter.Listener { args ->
        ...
        try {
            ...
            val massegeStatus =
                massegeDao!!.getMassegeStatus(sender_id, receiver_id, time, user_login_id)
            if (massegeStatus < 1) {
            ...
            if (socket != null) {
                if (socket!!.isActive) {
                    Log.d(
                        ...
                        "onAvailable || if cond isActive : true"
                    )
                    //                    socket.connect();
                } else {
                    CreateSocketConnection()
                    Log.d(
                        "log-ConnectivityManager.NetworkCallback",
                        "onAvailable || if cond isActive : false"
                    )
                }
            } else {
...
//                Toast.makeText(MainActivity.this, "Camera Permission Granted", Toast.LENGTH_SHORT).show();
                ...
                Toast.makeText(
                    this@MainActivity,
                    "Camera Permission needed to use the massenger",
                    Toast.LENGTH_SHORT
                ).show()
            }
...
//                Toast.makeText(MainActivity.this, "Storage Permission Granted", Toast.LENGTH_SHORT).show();
                ...
                Toast.makeText(
                    this@MainActivity,
                    "Storage Permission needed to use the massenger",
                    Toast.LENGTH_SHORT
                ).show()
            }
            ...
            if (grantResults.size > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                Toast.makeText(
                    this@MainActivity,
                    "To use Massenger please give all permissions",
                    Toast.LENGTH_SHORT
                ).show()
                //                initContentResolver();
//                this.finish();
            ...
            if (grantResults.size > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                Toast.makeText(
                    this@MainActivity,
                    "To use Massenger please give Contact and Storage permission",
                    Toast.LENGTH_SHORT
                ).show()
            }
...
//                initContentResolver();
                ...
                Toast.makeText(
                    this@MainActivity,
                    "To use Massenger you must give the Contact Read and Write permission, please restart the app",
                    Toast.LENGTH_SHORT
                ).show()
                //                initContentResolver();
            }
    ...
    companion object {
                    ...
                    JsonArrayRequest(Request.Method.POST, endpoint, mainArray, { response ->
                        ...
                        try {
                            ...
                            if (massegeDao.getContactWith_CID(CID, user_login_id) == null) {
                                ...
                                massegeDao.setPriorityRank(
                                    CID, massegeDao.getHighestPriorityRank(
                                        user_login_id
                                    ), user_login_id
                                )
                                Log.d(
        ...
        @JvmStatic
        fun saveContactProfileImageToStorage(
            CID: String,
...
//        Bitmap bitmapImage = BitmapFactory.decodeByteArray(profileImageByteArray, 0, profileImageByteArray.length);
            val bitmapImage =
                BitmapFactory.decodeByteArray(profileImageByteArray, 0, profileImageByteArray.size)
            val directory = File(
            ...
            val imagePath = File(directory, "" + CID + user_login_id + ".png")
            ...
            try {
                FileOutputStream(imagePath).use { outputStream ->
                    ...
                    Log.d(
                        "log-saveImageToInternalStorage",
                        "Saved image at path: " + imagePath.absolutePath
                    )
                    Log.d(
                        "log-saveImageToInternalStorage",
                        "Saved image of size : " + bitmapImage.width + "*" + bitmapImage.height
                    )
                    return true
                }
            } catch (e: IOException) {
