...
//                            Log.d("log-onCheckContactOnlineStatus_return", "call: enter in offline cond.");
                    if (onlineStatusPolicy == 0) {
                        ...
                        val date = Date(online_status)
                        //here we have to implement some function
                        val current_date = Date()
                        if (current_date.year == date.year && current_date.month == date.month && current_date.date == date.date) {
//                                    Log.d("log-onCheckContactOnlineStatus_return", "date: " + date);
                            val formatted = SimpleDateFormat("HH:mm").format(date)
                            //                                    Log.d("log-onCheckContactOnlineStatus_return", "formatted: " + formatted);
                            online_status_text_area!!.text = "last seen at $formatted"
                        } else if (current_date.year == date.year && current_date.month == date.month && current_date.date == date.date + 1) {
...
//                                    Log.d("log-onCheckContactOnlineStatus_return", "date: " + date);
                            ...
                            val cur_formatted =
                                SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(current_date)
                            online_status_text_area!!.text = "last seen at $formatted"
                        }
                    }
                } else {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Log.d(
            "log-ContactMassegeDetailsView-onCreate",
            "contact_id:" + CID + ", user_login_id:" + MainActivity.user_login_id
        )
        ...
        MainActivity.socket!!.on(
            ...
            onCheckContactOnlineStatus_return
        )
        ...
        massege_field!!.addTextChangedListener(object : TextWatcher {
            ...
            override fun onTextChanged(charSequence: CharSequence, i: Int, i1: Int, i2: Int) {
                if (massege_field!!.text.toString() != "") {
                    if (massege_field!!.text.toString().trim { it <= ' ' }.isBlank()) {
                        send_massege_button!!.isClickable = false
                    } else {
        ...
        activityRootView.viewTreeObserver.addOnGlobalLayoutListener {
            ...
            if (heightDiff > dpToPx(
                    this@ContactMassegeDetailsView,
                    200f
                )
                ...
                if (keyboardPass) {
                    ...
                    ContactMassegeRecyclerView?.scrollToPosition(
                        massegeRecyclerViewAdapter!!.itemCountMyOwn
                    )
                }
            } else {
                if (!keyboardPass) {
    ...
    fun setBackgroundImage() {
        val ti = Thread {
            val imagePath =
                "/storage/emulated/0/Android/media/com.massenger.mank.main/bg/bgImages/" + MainActivity.user_login_id + ".png"
    ...
    fun addNewUserIntoContact(view: View?) {
        ...
        contactIntent
            ...
            .putExtra(ContactsContract.Intents.Insert.PHONE, ContactMobileNumber.toString())
    ...
    private fun setNewMassegeArriveValue(cId: String) {
        val t =
            Thread { massegeDao!!.updateNewMassegeArriveValue(cId, 0, MainActivity.user_login_id) }
        t.start()
    }
    ...
    private fun setAllMassege(CID: String) {
        ...
        ContactMassegeRecyclerView!!.adapter = massegeRecyclerViewAdapter
    ...
    var LAUNCH_ContactDetailsFromMassegeViewPage_ACTIVITY = 1
    ...
    fun SendMassege(view: View?) {
        ...
        val setPriorityRankThread = Thread {
            ...
            if (MainActivity.contactListAdapter != null) {
                MainActivity.contactListAdapter!!.updatePositionOfContact(
                    CID,
                    this@ContactMassegeDetailsView
                )
            }
        ...
        Log.d(
            "log-try",
            "Send_massege_of_user: internet connection is : " + check_internet_connectivity()
        )
        if (check_internet_connectivity()) {
            massege_status =
                if (MainActivity.user_login_id == MainActivity.Contact_page_opened_id) {
                    3
                } else {
                    0
                }
            val new_massege = MassegeEntity(
                MainActivity.user_login_id,
                CID,
                user_massege,
                current_date.time,
