...
class ContactListAdapter(db: MainDatabaseClass) {
	...
	@SuppressLint("NotifyDataSetChanged", "RestrictedApi")
	fun addContact(newEntity: ContactWithMassengerEntity, flag: Int) { //now for only  unblocking
		MainActivity.contactList?.add(0, newEntity)
		recyclerViewAdapterNotifyLocal()
		setLastMassege(newEntity.CID)

		val r1 = contactDao?.updateUserBlockStatus(newEntity.CID.toString(), 0, MainActivity.user_login_id)
		Log.d("log-ContactListAdapter", "AddContact method start for ${newEntity.DisplayName}  and result is $r1  ")
	...
	fun setTypingStatus(CID: String) {
		...
		Log.d("log-ContactMassegeDetailsView", "updateOnlineStatusToTypingInUI start")        // Increment the counter
		typingCounter[CID] = (typingCounter[CID] ?: 0) + 1        // Calculate the delay based on the counter
		...
		handler.postDelayed({
			...
			typingCounter[CID] = (typingCounter[CID] ?: 0) - 1            // Check if it's the last run
	...
	fun blockContact(CID: String) {
		MainActivity.socket?.emit("contactBlockStatusChanged", user_login_id, CID, 1)
		Thread {
			val updateStatus = contactDao?.updateUserBlockStatus(CID, 1, user_login_id) //1 for block 0 for unblock
			Log.d("log-ContactListAdapter", "blockContact CID $CID abd $user_login_id and updateStatus $updateStatus")
		}.start()
		val contact = MainActivity.contactList?.find { it?.CID == CID }
