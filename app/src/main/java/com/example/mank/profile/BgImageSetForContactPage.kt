...
class BgImageSetForContactPage() : Activity() {
    ...
    fun bgSelectImageButtonOnClick(view: View?) {
        ...
        Log.d(
            ...
            "bgSelectImageButtonOnClick method before startActivityForResult"
        )
        @SuppressLint("IntentReset") val pickIntent =
    ...
    fun bgSaveButtonOnClick(view: View?) {
        if (!blockSaveButton) {
            Log.d(
                "log-BgImageSetForContactPage",
                "bgSaveButtonOnClick blockSaveButton$blockSaveButton"
            )
            if (passForSave) {
//                Bitmap
    ...
    private fun saveImageToInternalStorage(bitmapImage: Bitmap?) {
        ...
        val directory = File(
            ...
            "Android/media/com.massenger.mank.main/bg/bgImages"
        )
        if (!directory.exists()) {
        ...
        try {
            FileOutputStream(imagePath).use { outputStream ->
                ...
                Log.d(
                    ...
                    "Saved image path: " + imagePath.getAbsolutePath()
                )
                runOnUiThread(object : Runnable {
                    override fun run() {
                        Toast.makeText(
                            this@BgImageSetForContactPage,
            ...
            runOnUiThread(object : Runnable {
                override fun run() {
                    Toast.makeText(
                        this@BgImageSetForContactPage,
                        "background image is update failed",
                        Toast.LENGTH_SHORT
                    ).show()
                    progressBar!!.visibility = View.GONE
                }
